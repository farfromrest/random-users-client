...
describe('SortBox', () => {
  it('should indicate unsorted by default', () => {
    const { getByText } = render(<SortBox />)
    getByText('Unsorted')
  })

  it('should indicate sorted by name when sortBy is name', () => {
    const { getByText } = render(<SortBox sortBy={SORT_BY.NAME} />)
    getByText('Sorted by name')
  })

  it('should indicate sorted by name in reverse when sortBy is name reverse', () => {
